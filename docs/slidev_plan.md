...
ただし、Slidev v52 系では Pages の base path と hash routing の組み合わせで、矢印キー遷移時に `#/presenter/presenter/7` のような重複 hash が発生することがある。`setup/routes.ts` で presenter の重複 route を `#/presenter/7` へ redirect し、それ以外の未知 route は `#/1` へ戻している。GitHub Pages まわりを触る場合は、通常表示と presenter 表示の両方で矢印キー遷移を確認する。
