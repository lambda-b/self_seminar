...
全テーマを深く証明するのは難しいため、今回は「測度論から確率論への地図」を主目的にする。
...
大数の法則・中心極限定理・確率過程・条件付き期待値・Brown運動・Markov性・Martingaleは本編の柱として扱う。伊藤積分の構成は紹介程度に留め、伊藤の公式で2階微分項が残る意味と、Black-Scholesや最適停止への接続を見せる。Black-Scholesでは、自己資金戦略と伊藤の公式による2通りの $dV_t$ を比較し、Black-Scholes方程式とコールオプション価格式までを見取り図として示す。秘書問題は最適停止の応用例として紹介し、導出や証明を厚くする場合は補助ノートに分ける。
