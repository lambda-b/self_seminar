...
ただし、Slidev v52 系では Pages の base path と hash routing の組み合わせで、矢印キー遷移時に `#/self_seminar/2` や `#/presenter/presenter/7` のような重複 hash が発生することがある。`setup/routes.ts` でこれらを `#/2`、`#/presenter/7` へ redirect し、404 表示に落ちる前に正規ルートへ戻している。GitHub Pages まわりを触る場合は、通常表示と presenter 表示の両方で矢印キー遷移を確認する。
