...
$A=\{\omega\in\Omega:X_1(\omega)=1\}$ とする。

$$
\mathcal{G}=\sigma(A)=\{\emptyset,A,A^c,\Omega\}
$$
...
$E[X\mid\mathcal{G}]$ は、$\mathcal{G}$ で区別できる情報だけで作った $X$ の予測。

$L^2$ の言葉では、$\mathcal{G}$ 可測な確率変数の空間への射影と見られる。

::note
$E[S_2\mid X_1=1]=4.5$ は事象 $A$ 上での値。$\sigma(A)$ で条件付けた確率変数全体は、$A^c$ 上の値も含めて書く。
::
