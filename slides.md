...
採用した候補の最終順位を $R_\tau$、順位 $r$ の「嬉しさ」を $u(r)$ とする。

$$
\max_{\tau\in\mathcal T}E[u(R_\tau)]
=\max_{\tau\in\mathcal T}\sum_{r=1}^n u(r)P(R_\tau=r)
$$

- 古典的秘書問題：$u(r)=\mathbf 1_{\{r=1\}}$
- 上位3人なら同等：$u(r)=\mathbf 1_{\{r\le3\}}$
- 順位に点数をつける：$u(1)\ge u(2)\ge\cdots\ge u(n)$

評価関数を変えると、採用すべき時期と相対順位も変わる。

<SimulatorLink />

::note
シミュレータでは候補者数、順位ごとの効用、現在の進捗と相対順位を入力できる。条件付き確率の式や後ろ向き計算の詳細も補足ページ側に掲載している。
::
...
$k$ 人目の相対順位を $S_k$ とする。今採用する期待効用は

$$
G_{k,S_k}=E[u(R_k)\mid S_k]
$$

この先も探す期待効用を $V_{k+1}$ とすれば、最適戦略は

$$
\boxed{\quad
\tau^*=\inf\left\{k:G_{k,S_k}\ge V_{k+1}\right\}
\quad}
$$

評価関数が変わっても「今採用する価値」と「待つ価値」を比較する。

<SimulatorLink />

::note
継続価値は $V_k=k^{-1}\sum_{s=1}^k\max\{G_{k,s},V_{k+1}\}$ と後ろ向きに求める。詳しい条件付き確率と漸化式はシミュレータ内で紹介する。
::
