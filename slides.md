...
::small-text{class="text-gray-500"}
...
の極限としても現れる。1ステップの大きさが $\sqrt{dt}$ なので、後で $(dW_t)^2=dt$ という記号計算につながる。
::
...
\quad
\Phi(x) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^x e^{-\frac{\xi^2}{2}} \, d\xi
$$
...
情報増大系 $(\mathcal{F}_t)$ に対して、すべての $t$ で

$$
\{\tau \le t\}\in\mathcal{F}_t
$$

なら、時刻 $t$ までの情報だけで「もう止めたか」を判定できる。
...
::example-box{title="例: 停止時刻とそうでないもの"}
サイコロの出目を $X_1,X_2,\ldots$、時刻 $n$ までの情報を $\mathcal{F}_n$ とする。
...
時刻 $n$ までに6が出たかは、時刻 $n$ までの出目だけで判定できる。
...
$\{\tau\le1\}=\{X_2=6\}$ は、時刻1の情報 $\mathcal{F}_1$ では判定できない。
::
